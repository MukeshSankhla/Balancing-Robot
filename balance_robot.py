...
# --- Main Program ---
def main():
    # Detect platform for default port assignments (Ubuntu/Linux vs. Windows)
    is_linux = sys.platform.startswith('linux')
    default_imu = "/dev/ttyACM0" if is_linux else "COM10"
    default_motor = "/dev/ttyACM1" if is_linux else "COM13"

    parser = argparse.ArgumentParser(description="Self-Balancing Robot Controller")
    parser.add_argument("--imu", type=str, default=None, help=f"IMU (RP2040) serial port. Falls back to --esp (default: {default_imu}).")
    parser.add_argument("--esp", type=str, default=default_imu, help=f"ESP32 serial port (deprecated, use --imu instead, default: {default_imu})")
    parser.add_argument("--motor", type=str, default=default_motor, help=f"Motor RS485 serial port (default: {default_motor})")
